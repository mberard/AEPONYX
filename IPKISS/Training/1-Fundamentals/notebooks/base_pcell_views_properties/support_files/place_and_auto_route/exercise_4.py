...
links = [# input coupler to splitter
         ...
         ("gc_pass:out",    "ring8:out1")]


for cnt in range(2,9):
    p1 = "ring{}:out1".format(cnt-1)
    p2 = "ring{}:in1".format(cnt)
    links.append((p1,p2))
...
transformations['gc_ref'] = i3.vector_match_transform(grating_coupler_layout.ports["out"], i3.Vector(position=(9*d,-d/2), angle=0.0))
transformations['gc_pass'] = i3.vector_match_transform(grating_coupler_layout.ports["out"], i3.Vector(position=(9*d,d/2), angle=0.0))
...
for cnt in range(1,9):
    plt.plot(wavelengths, 20 * np.log10(np.abs(S['gc_in_vertical_in', 'gc_out{}_vertical_in'.format(cnt)])), label='out{}'.format(cnt))
