...
class Circuit(APAC):
    ...
    def get_cr1(self):
        ...
        t1 = i3.Translation(insts["bp1"].size_info().south_west) - i3.Translation((0, 20))
        t2 = i3.Translation(insts["bp2"].size_info().north_west) + i3.Translation((0, 20))
        cr = combine_connectors([sbend, manhattan, manhattan], [t1, t2])
        return cr

    def get_cr2(self):
        insts = self.get_child_instances()
        t1 = i3.Translation((insts["bp3"].size_info().north_west)) + i3.Translation((0, 20))
        cr = combine_connectors([sbend, manhattan], [t1])
        return cr

    def _default_connectors(self):
        connectors = [("bend1a:out", "bend1b:out"),
                      ("spl2:in1", "grout:out", self.get_cr1()),
    ...
    class Layout(APAC.Layout):
        ...
        def _default_child_transformations(self):
            return {"spl1": (0.0, 0.0),
                    "grin": (-200, -200),
                    "grout": i3.HMirror() + i3.Translation(translation=(600, +150)),
                    "bp1": (300, 0),
                    "bp2": (500, 150),
                    "bp3": (-100, -100)}
